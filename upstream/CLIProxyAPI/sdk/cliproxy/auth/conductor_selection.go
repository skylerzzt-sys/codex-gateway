...
// availableAuthsForSelector reports the highest-priority candidates for native selection, the
// selector candidates, and the candidates handed to the plugin scheduler. The plugin view keeps
// all available priority tiers so an explicit routing policy can select a lower-priority auth.
func (m *Manager) availableAuthsForSelector(selector Selector, auths []*Auth, provider, routeModel string, now time.Time) (priorityAuths, selectorAuths, pluginAuths []*Auth, err error) {
	// One availability pass serves all consumers. Native selection keeps its existing
	// highest-priority view, while plugin schedulers also need lower-priority candidates so
	// an explicit routing policy can select them.
	allAuths, err := m.availableAuthsForRouteModelAcrossPriorities(auths, provider, routeModel, now)
	if err != nil {
		return nil, nil, nil, err
	}
	allAuths = cloneAuthSlice(allAuths)
	pluginAuths = cloneAuthSlice(allAuths)
	...
	return priorityAuths, selectorAuths, pluginAuths, nil
}
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	_, selectorAuths, pluginAuths, errAvailable := m.availableAuthsForSelector(selector, candidates, provider, model, time.Now())
	if errAvailable != nil {
		m.mu.RUnlock()
	...
	selected, handled, errPick := m.pickViaPluginScheduler(ctx, pluginScheduler, provider, []string{provider}, model, opts, tried, pluginAuths)
	if errPick != nil {
...
func (m *Manager) pickNextMixedLegacy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	_, selectorAuths, pluginAuths, errAvailable := m.availableAuthsForSelector(selector, candidates, "mixed", model, time.Now())
	if errAvailable != nil {
		m.mu.RUnlock()
		return nil, nil, "", errAvailable
	}
	...
	selected, handled, errPick := m.pickViaPluginScheduler(ctx, pluginScheduler, "mixed", providers, model, opts, tried, pluginAuths)
	if errPick != nil {
		return nil, nil, "", errPick
	}
	if !handled {
