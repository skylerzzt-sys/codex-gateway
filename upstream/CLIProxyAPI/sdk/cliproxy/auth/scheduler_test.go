...
func TestManagerPluginSchedulerCanForceLowerPriorityAuth(t *testing.T) {
	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	manager.executors["codex"] = schedulerTestExecutor{}
	for _, auth := range []*Auth{
		{ID: "auth-high", Provider: "codex", Status: StatusActive, Attributes: map[string]string{"priority": "10"}},
		{ID: "auth-low", Provider: "codex", Status: StatusActive, Attributes: map[string]string{"priority": "1"}},
	} {
		if _, errRegister := manager.Register(context.Background(), auth); errRegister != nil {
			t.Fatalf("Register(%s) error = %v", auth.ID, errRegister)
		}
	...
	scheduler := &fakePluginScheduler{
		...
		pick: func(ctx context.Context, req pluginapi.SchedulerPickRequest) (pluginapi.SchedulerPickResponse, bool, error) {
			...
			seen := make(map[string]int, len(req.Candidates))
			for _, candidate := range req.Candidates {
				seen[candidate.ID] = candidate.Priority
			}
			if seen["auth-high"] != 10 || seen["auth-low"] != 1 {
				t.Fatalf("scheduler candidates = %#v, want both priority tiers", seen)
			}
			return pluginapi.SchedulerPickResponse{Handled: true, AuthID: "auth-low"}, true, nil
		},
	}
	manager.SetPluginScheduler(scheduler)
	...
	if got == nil || got.ID != "auth-low" {
		t.Fatalf("pickNext() auth = %#v, want auth-low", got)
	}
}

func TestManagerPluginSchedulerUnhandledKeepsNativePrioritySelection(t *testing.T) {
	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	manager.executors["codex"] = schedulerTestExecutor{}
	for _, auth := range []*Auth{
		{ID: "auth-high", Provider: "codex", Status: StatusActive, Attributes: map[string]string{"priority": "10"}},
	...
	if got == nil || got.ID != "auth-high" {
		t.Fatalf("pickNext() auth = %#v, want auth-high", got)
	}
	if len(scheduler.requests) != 1 || len(scheduler.requests[0].Candidates) != 2 {
		t.Fatalf("plugin scheduler candidates = %#v, want both priority tiers", scheduler.requests)
	}
}
